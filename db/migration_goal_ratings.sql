...
-- it to the 0-10 integer the rest of the app uses via a per-competition,
-- manually-configured percentile -> score mapping. The percentile distribution
-- is computed from the Goal raw ratings already stored for that competition
-- (no cross-season/historical dataset is wired up yet).
...
alter table public.competitions
    ...
    add column if not exists rating_mapping    jsonb;                          -- {"0-20":4,...}; null -> code default
